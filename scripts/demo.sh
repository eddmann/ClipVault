#!/bin/bash
# Demo mode launcher for ClipVault (macOS)
...
# Demo modes
MODES=(
    "empty|Empty clipboard history"
    "withHistory|Populated with sample clipboard items"
    "pinnedItems|History with some pinned items"
    "historyWindow|History window open with items"
    "historyWindowEmpty|History window open, empty state"
)

echo "=== ClipVault Demo Mode Launcher ==="
echo ""
...
xcodebuild -project "$PROJECT_DIR/ClipVault.xcodeproj" \
    -scheme "$SCHEME" \
    -configuration Debug \
...
# Find the built app
APP_PATH=$(find "$BUILD_DIR/Build/Products" -name "ClipVault.app" -type d 2>/dev/null | head -1)
...
pkill -f "ClipVault.app" 2>/dev/null || true
sleep 0.5
